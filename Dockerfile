...
ENTRYPOINT ["sh", "-c", "/app/wait-for-database.sh && /app/wait-for-redis.sh && cat /app/resources/application.properties && java -jar app.jar --spring.config.location=/app/resources/application.properties"]